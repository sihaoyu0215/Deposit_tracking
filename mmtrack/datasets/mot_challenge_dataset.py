...
class MOTChallengeDataset(CocoVideoDataset):
    ...
    # CLASSES = ('pedestrian', ) Change by Stephen Yu for pipe task
    CLASSES = ('Deposit', )
    ...
    def get_benchmark_and_eval_split(self):
        ...
        Get benchmark from upper/lower-case image prefix and the dataset
        split to evaluate.
    ...
    def evaluate(self,
        ...
        if 'track' in metrics:
            ...
            hota = [
                np.average(output_res['MotChallenge2DBox']['track'][seq]
                           ['Deposit']['HOTA']['HOTA']) for seq in seq_list
                if 'OVERALL' not in seq
            ]
